...
List<DataPoint> simpleMovingAverage(List<DataPoint> rawData, int parameter) {
  List<DataPoint> smoothedData = [];

  // Iterate through the data points, starting from the (parameter - 1)-th index.
  for (int i = parameter - 1; i < rawData.length; i++) {
    double sum = 0.0;
    ...
    smoothedData.add(DataPoint(x: rawData[i].x, y: sum / parameter));
  }
